...
Tcalc<-function(GT,RGT,FGT,NGT,nsim,nages,npop,nfleets,nareas,y,m,nyears){
  ...
  frac<-NTAL/(NTAL+NGT) # binomial prop recap [sim, pop, age, area], note the denominator is not (NGT + NTAL) - Tagged individuals are 'in' the bulk transfer N
  ...
  if(GTcheck){
    print(paste("max fraction tagged = ",round(max(frac),4)))
  ...
  for(i in 1:nsim){ # has  to be by sim because of ragged tag release indexing
    ...
    nTALy<-length(TALni)
    reltab<-cbind(TEG(c(nareas,npop,nages)),as.vector(rels2[i,,,]))
  ...
  for(i in 1:nsim){
     ...
     if(recapbysim[i]!=0){
       ...
       for(j in 1:nrow(rectab)){
           ...
           recapTagNo<-sample(TALtemp[,1],rectab[j,4],replace=FALSE,prob=TALtemp[,5])
           pop<-rectab[j,2]
           area<-rectab[j,1]
...
FishCap<-function(GT,FGT,NGT,nsim,nages,npop,nfleets,nareas,y,m,nyears){
  ...
  for(i in 1:nsim){
    ...
    Tind<-(1:Tindex[i])[apply(TAL[,i,,],1,sum)>0]
    nTi<-length(Tind)
