...
#' }
setClass("MSE_GT",representation(
...
setMethod("initialize", "MSE_GT", function(.Object,OM=OM_example,MPs=list(c("U5","U5")),
  ...
  if(is.null(GT))stop("No Gene tagging specifications 'GT' added to this run")  # are Gene tagging calculations to be carried out?
    ...
    for(y in nyears:(nyears+proyears)){ # for(y in nyears:(upyrs[1]-1)){
      ...
      if(y%in%upyrs){# Operate MP S P A Y M R
        ...
        for(AS in 1:nAss){
          ...
          upind<-(nyears+1):(y-1)
          if(length(upind)>0){
            CN<-apply(array(C[,,,upind,,AA,],c(nsim,npop,nages,length(upind),nsubyears,nA,nfleets)),c(1,4),sum,na.rm=T)*.Object@Cerr[,upind]
          }
      ...
      for(m in 1:nsubyears){
        ...
        GT_MP<-calc_GT(GT=GT_MP,y=y, m=m, N=N, FM=FM, M=M, mov=mov,
                             nsubyears=nsubyears, movIndex=movIndex, nsim=nsim,
                             nages=nages,npop=npop,nyears=nyears,nareas=nareas,
                             nfleets=nfleets)
