...
Brownie <- function(tag_data, fix_M = FALSE, M = 0.06, latency = 4, report_rate = 1,
  ...
  #TMB::compile("C:/GitHub/ABTGT/ABTGT/R/Brownie/brownie.cpp")
...
plot_F_Brownie <- function(Brownie, ylim = c(0, 0.1)) {
  ...
  plot_timeseries(yrs, Brownie$report$F, obs_lower = exp(FF_lower), obs_upper = exp(FF_upper))
}

plot_Brownie_fit <- function(Brownie, diag = TRUE) {
  Find <- rownames(Brownie$SD$cov.fixed) == "log_F"
  yrs<-1:(sum(Find))
  plot_tags <- function(Year_rel = yrs, Year_recap = yrs, obs, fit = NULL, N_rel,
...
plot_Brownie_resid <- function(Brownie, bubble_adj = 5) {
  ...
  plot_resid <- function(Year_rel = yrs, Year_recap = yrs, obs, fit = NULL, N_rel,
                         bubble_adj) {
